...
namespace ros_control_gazebo_tests {
  ...
  class RobotSimParallel : public ros_control_gazebo::RobotSim
  {
  public:
    RobotSimParallel() :
      n_dof_(5),
      ...
      joint_velocity_command_(n_dof_)
    {
      ...
      joint_name_[3] = "joint4";
      joint_name_[4] = "joint5";

      for(unsigned int j=0; j < n_dof_; j++) 
      {
        joint_position_[j] = 1.0;
        joint_velocity_[j] = 0.0;
        joint_effort_[j] = 0.0;//0.1 previously
    ...
    void writeSim(ros::Time time, ros::Duration period) 
    {
      for(unsigned int j=0; j < n_dof_; j++) 
      {
        ...
        //Four-Bar Mechanisim for Keeping links parallel to each other
        //-----joint4 mimics joint3---- 
        sim_joints_[3]->SetAngle(0,joint_position_[1]+joint_position_[2]);   
        //-----joint5 mimics joint2---- 
        sim_joints_[4]->SetAngle(0,((-joint_position_[3]+joint_position_[1])));  
...
PLUGINLIB_DECLARE_CLASS(ros_control_gazebo_tests, RobotSimParallel, ros_control_gazebo_tests::RobotSimParallel, ros_control_gazebo::RobotSim)
